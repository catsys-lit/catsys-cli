...
const createApp = tagName => {
  ...
  createImages(path);
  createPolymerFile(path, tagName);
  createManifestFile(path, tagName, className);
  createReadmeFile(path, tagName);
...
const createPolymerFile = (path, tagName) => {
  try {
    let file = fs.readFileSync(`${__dirname}/templates/polymer.tpl`, 'utf8');
    file = file.replace(/{{APP_TAG}}/gm, tagName);
    createFile(path, 'polymer.json', file);
  } catch (error) {
    console.error(error);
  }
};

const createReadmeFile = (path, tagName) => {
  try {
...
const createManifestFile = (path, className) => {
  try {
    let file = fs.readFileSync(`${__dirname}/templates/manifest.tpl`, 'utf8');
    file = file.replace(/{{APP_CLASS}}/gm, className);
    createFile(path, 'manifest.json', file);
  } catch (error) {
    console.error(error);
  }
};
